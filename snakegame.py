...
pygame.display.set_caption('Snake Game by Ahunjan')
...
class Snake():
    ...
    def draw(self):
        ...
        for x in self.snake_List:
            i = i + 1
            if len(self.snake_List) > 1 and len(self.snake_List) == i:
    ...
    def is_beats_wall(self,level):
        for wall in level_walls[level]:
            if self.x1 >= wall["start_x"] and self.y1 >= wall["start_y"] and self.x1 < wall["start_x"] + wall["weight_x"] and self.y1 < wall["start_y"] + wall["weight_y"]: 
...
def check_food_for_wall(level,foodx,foody):
    for wall in level_walls[level]:
        if foodx + snake_block >= wall["start_x"] and foody + snake_block >= wall["start_y"] and foodx + snake_block <= wall["start_x"] + wall["weight_x"] and foody + snake_block <= wall["start_y"] + wall["weight_y"]: 
            foodx = round(random.randrange(0, wall["start_x"] - snake_block) / 10.0) * 10.0
            foody = round(random.randrange(0, wall["start_y"] - snake_block) / 10.0) * 10.0
            return [foodx,foody]
    return [foodx,foody]
wall1 = {
  "start_x": 150,
    ...
    while not game_over:
        while game_close == True:
            ...
            if player1.score > player2.score:
                message("Player 1 wins! "+str(player1.score) + ":"+ str(player2.score) +" Press C-Play Again or Q-Quit", yellow)
            elif player1.score < player2.score:
                message("Player 2 wins! "+str(player1.score) + ":"+ str(player2.score) +" Press C-Play Again or Q-Quit", yellow)
            else:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_c:
                    gameLoop(0)
                if event.key == pygame.K_LEFT and player1.direction != 'RIGHT':
                ...
                elif event.key == pygame.K_UP and player1.direction != 'DOWN':
